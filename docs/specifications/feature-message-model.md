...
### Phase 1: Message 数据模型 [核心基础] [待开始]

所有后续阶段依赖此阶段。重写 `messages.py`，替换旧的 Message/ToolCall/ToolResult。

- [ ] **Task 1.1**: 重写 `src/mutagent/messages.py` — ContentBlock 体系 + 新 Message
  - [ ] 定义 ContentBlock 基类（`type: str`）
  - [ ] 定义 TextBlock、ImageBlock、DocumentBlock、ThinkingBlock、ToolUseBlock
  - [ ] 定义新 Message（role, blocks, id, label, sender, model, timestamp, duration, input_tokens, output_tokens, cacheable, priority）
  - [ ] 移除旧 ToolCall、ToolResult、Message
  - [ ] 更新 Response — `message: Message` 使用新 Message
  - [ ] 更新 StreamEvent — `tool_call` 字段类型改为 `ToolUseBlock | None`，移除 `tool_result` 字段
  - [ ] 保留 ToolSchema、InputEvent、Content 不变
  - 状态：⏸️ 待开始

### Phase 2: AgentContext Declaration [待开始]

- [ ] **Task 2.1**: 创建 `src/mutagent/context.py` — AgentContext Declaration
  - [ ] `context_window: int`（0 = 未知）
  - [ ] `prompts: list[Message] = []`
  - [ ] `messages: list[Message] = []`
  - [ ] `prepare_prompts() -> list[Message]`
  - [ ] `prepare_messages() -> list[Message]`
  - [ ] `update_usage(usage: dict[str, int]) -> None`
  - [ ] `get_context_used() -> int`
  - [ ] `get_context_percent() -> float | None`
  - 状态：⏸️ 待开始

- [ ] **Task 2.2**: 创建 `src/mutagent/builtins/context_impl.py` — 默认实现
  - [ ] `prepare_prompts`: 按 priority 降序排列，过滤
  - [ ] `prepare_messages`: 默认直接返回 messages
  - [ ] `update_usage / get_context_used / get_context_percent`: token 用量追踪
  - [ ] 在 context.py 底部注册 impl 模块
  - 状态：⏸️ 待开始

### Phase 3: LLMProvider & LLMClient 签名更新 [待开始]

Provider 和 Client 的 `system_prompt: str` 参数改为 `prompts: list[Message]`，由 Provider 负责转换。

- [ ] **Task 3.1**: 更新 `src/mutagent/provider.py`
  - [ ] `send()` 参数: `system_prompt: str` → `prompts: list[Message]`
  - 状态：⏸️ 待开始

- [ ] **Task 3.2**: 更新 `src/mutagent/client.py`
  - [ ] `send_message()` 参数: `system_prompt: str` → `prompts: list[Message]`
  - 状态：⏸️ 待开始

- [ ] **Task 3.3**: 更新 `src/mutagent/builtins/client_impl.py`
  - [ ] `send_message` 实现适配新签名
  - [ ] API 录制逻辑适配新 Message 格式
  - 状态：⏸️ 待开始

### Phase 4: Provider 转换逻辑重写 [待开始]

两个 Provider 的核心转换逻辑全部重写，处理 blocks 模型。

- [ ] **Task 4.1**: 重写 `src/mutagent/builtins/anthropic_provider.py`
  - [ ] `_messages_to_claude`: blocks → content 数组；含已完成 ToolUseBlock 的 assistant 消息 → 拆分为 tool_use(assistant) + tool_result(user)；ThinkingBlock → thinking/redacted_thinking；ImageBlock/DocumentBlock 映射
  - [ ] prompts → `system` 字段（content block 数组），`cacheable=True` → `cache_control`
  - [ ] 流式解析: 处理 thinking/redacted_thinking content_block_start/delta
  - [ ] `_response_from_claude`: 解析 thinking blocks，构建新 Message(blocks=[...])
  - 状态：⏸️ 待开始

- [ ] **Task 4.2**: 重写 `src/mutagent/builtins/openai_provider.py`
  - [ ] `_messages_to_openai`: blocks → content 数组；ToolUseBlock → tool_calls + role:"tool" 消息；忽略 ThinkingBlock
  - [ ] prompts → `role: "system"` 消息
  - [ ] `_response_from_openai`: 构建新 Message(blocks=[...])
  - 状态：⏸️ 待开始

### Phase 5: Agent Declaration & ToolSet 更新 [待开始]

- [ ] **Task 5.1**: 更新 `src/mutagent/agent.py`
  - [ ] `client` → `llm: LLMClient`
  - [ ] `tool_set` → `tools: ToolSet`
  - [ ] 新增 `context: AgentContext`
  - [ ] 移除 `system_prompt`、`messages`、`max_tool_rounds`
  - [ ] `handle_tool_calls` 签名: `(tool_calls: list[ToolUseBlock]) -> None`（原地更新 block，不返回 list）
  - 状态：⏸️ 待开始

- [ ] **Task 5.2**: 更新 `src/mutagent/tools.py` + `builtins/tool_set_impl.py`
  - [ ] `dispatch` 签名: 接收 `ToolUseBlock`，执行后直接更新 block 的 status/result/is_error/duration
  - [ ] 不再返回 ToolResult（ToolResult 已移除）
  - 状态：⏸️ 待开始

### Phase 6: Agent 实现重写 [待开始]

- [ ] **Task 6.1**: 重写 `src/mutagent/builtins/agent_impl.py`
  - [ ] `run()`: 使用 `self.context.messages` 替代 `self.messages`；用户消息构建 `Message(role="user", blocks=[TextBlock(text=...)])`
  - [ ] `step()`: 调用 `self.context.prepare_prompts()` + `self.context.prepare_messages()` → `self.llm.send_message(messages, tools, prompts=prompts)`；响应后调用 `self.context.update_usage()`
  - [ ] `handle_tool_calls()`: 遍历 ToolUseBlock，设置 status="running" → 调用 `self.tools.dispatch(block)` → block 自动更新；不再拼装独立 tool_result 消息
  - [ ] 工具结果不再以独立 user Message 添加（ToolUseBlock 在 assistant Message 上原地更新，Provider 发送时自动生成 tool_result）
  - 状态：⏸️ 待开始
...
- [ ] **Task 7.1**: 更新 import 站点
  - [ ] `builtins/delegate_impl.py` — 适配新 Message + Agent 字段名
  - [ ] `builtins/main_impl.py` — 适配新 Message + Agent 字段名
  - [ ] `userio.py` / `builtins/userio_impl.py` / `extras/rich/userio_impl.py` — StreamEvent 字段变更
  - [ ] `toolkits/web_toolkit.py` / `builtins/web_toolkit_impl.py` — ToolSchema 不变，无需改动（确认）
  - [ ] `builtins/schema.py` — ToolSchema 不变（确认）
  - [ ] `__init__.py` — 考虑是否导出 AgentContext
  - 状态：⏸️ 待开始

### Phase 8: 测试 [待开始]

- [ ] **Task 8.1**: 重写 `tests/test_messages.py` — 新 Message/ContentBlock 测试
  - 状态：⏸️ 待开始

- [ ] **Task 8.2**: 新增 `tests/test_context.py` — AgentContext 测试
  - 状态：⏸️ 待开始

- [ ] **Task 8.3**: 重写 `tests/test_openai_provider.py` — 基于 blocks 的转换测试
  - 状态：⏸️ 待开始

- [ ] **Task 8.4**: 更新 `tests/test_claude_impl.py` — 基于 blocks 的转换测试
  - 状态：⏸️ 待开始

- [ ] **Task 8.5**: 更新 `tests/test_agent.py` — 新 Agent 字段 + 新消息流转
  - 状态：⏸️ 待开始

- [ ] **Task 8.6**: 更新 `tests/test_e2e.py` — 端到端测试适配
  - 状态：⏸️ 待开始

- [ ] **Task 8.7**: 更新 `tests/test_tool_set.py` — dispatch 新签名
  - 状态：⏸️ 待开始

- [ ] **Task 8.8**: 更新其余测试文件（test_rich_extras, test_userio 等）
  - 状态：⏸️ 待开始

### Phase 9: 构建验证 [待开始]

- [ ] **Task 9.1**: 运行全部单元测试 `pytest`
  - 状态：⏸️ 待开始

- [ ] **Task 9.2**: 运行类型检查 `mypy src/mutagent`
  - 状态：⏸️ 待开始
